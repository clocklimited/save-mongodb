...
function createEngine(collection, engineOptions) {
  ...
  function create(object, callback) {
    ...
    var entity = Object.assign({}, object)
    collection.insertOne(entity, function(error, res) {
      if (error) return callback(error)
      entity[options.idProperty] = res.insertedId
      var data = objectIdToString(entity)
  ...
  function find(query, saveOptions, callback) {
    ...
    if (callback === undefined) {
      ...
      var cursor = collection.find(castIdProperty(query), options)
